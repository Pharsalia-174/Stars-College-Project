...
public:
    ...
    int godMove(int x,int y);//可以跳出设置的范围直接改变棋子位置 可将棋子置于棋盘外 返回0 移动成功 1没移动 2 战斗后 刷新棋盘 不移动 3 发生战斗 刷新 移动
    int moveMob(int toward);//默认移动函数 步长为1 方向为 2↓ 4← 6→ 8↑ 返回0 移动成功 1没移动 2 战斗后 刷新棋盘 不移动  3 发生战斗 刷新 移动
